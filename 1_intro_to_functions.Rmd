...
```{r}
ggplot_funct <- function(data){ggs <- ggplot(data,aes(x=Date,y=q_cfs,color=site)) + 
  geom_line() 
return(ggs)
}
ggplot_funct(combo_data)
```
...
It didn't work for Texas site or Mississippi because they named thier data different than Colorado or New Mexico. Avoid hard coding ideally.

```{r}
colorado_q <- q_downloader(site_no = '07096000',
                          site_id = 'ARKANSAS RIVER AT CANON CITY, CO.')
nmexico_q <- q_downloader(site_no = '08383500',
                          site_id = 'PECOS RIVER NEAR PUERTO DE LUNA, NM')
```
...
```{r}
combo_data_co_nm <- bind_rows(colorado_q,nmexico_q)

join_two_sets <- function(data_left,data_right){
  combo_data_new <- bind_rows(data_left,data_right) %>%

  select(-site_no,-q_cd,-agency_cd) %>%
  spread(key=site,value=q_cfs)
    
}
new_combo_data <- join_two_sets(colorado_q,nmexico_q)
```
...
```{r}
xts_maker_graph <- function(df_wide,columnNameOne,columnNameTwo){
  wide_data <- df_wide

  wide_xts <- xts(wide_data %>%
                  select(columnNameOne,columnNameTwo),
                order.by = wide_data$Date)
  
  dygraph(wide_xts)
}

xts_maker_graph(new_combo_data,'ARKANSAS RIVER AT CANON CITY, CO.','PECOS RIVER NEAR PUERTO DE LUNA, NM')
```
